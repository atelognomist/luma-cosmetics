...
import { DEMO_PRODUCTS, formatPrice, formatDate, type Product } from "@/lib/data";
...
export default function Inventory() {
  ...
  const outCount = DEMO_PRODUCTS.filter((p) => p.stock === 0).length;
  const lowCount = DEMO_PRODUCTS.filter((p) => p.stock > 0 && p.stock <= p.lowStockThreshold).length;
    ...
    <div className="flex-1 flex flex-col min-h-0">
      <div className="px-7 py-5 bg-card" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        <div className="flex gap-0.5">
          {([
            ["all", "Tous", DEMO_PRODUCTS.length],
            ["out", "Rupture", outCount],
            ["low", "Stock faible", lowCount],
            ["in_stock", "En stock", DEMO_PRODUCTS.filter((p) => p.stock > p.lowStockThreshold).length],
