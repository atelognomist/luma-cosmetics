...
import { DEMO_ORDERS, DEMO_PRODUCTS, ANALYTICS_DAILY, formatPrice, orderTotal } from "@/lib/data";
...
const confirmedOrders = DEMO_ORDERS.filter((o) => ["confirmed", "preparing", "ready", "sent", "picked_up", "out_for_delivery", "delivered"].includes(o.status));
const deliveredOrders = DEMO_ORDERS.filter((o) => o.status === "delivered");
const rejectedOrders = DEMO_ORDERS.filter((o) => o.status === "rejected");
const noAnswerOrders = DEMO_ORDERS.filter((o) => o.status === "no_answer");
const cancelledOrders = DEMO_ORDERS.filter((o) => ["cancelled", "failed", "returned"].includes(o.status));

const grossValue = DEMO_ORDERS.reduce((s, o) => s + orderTotal(o), 0);
const confirmedValue = confirmedOrders.reduce((s, o) => s + orderTotal(o), 0);
const deliveredValue = deliveredOrders.reduce((s, o) => s + orderTotal(o), 0);
const cancelledValue = cancelledOrders.reduce((s, o) => s + orderTotal(o), 0);

const confRate = DEMO_ORDERS.length > 0 ? ((confirmedOrders.length / DEMO_ORDERS.length) * 100).toFixed(1) : "0";
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>
              Toutes les données sont démo — elles ne reflètent pas de vraies transactions
            </p>
          </div>
      ...
      <div className="flex-1 overflow-auto px-7 py-6 flex flex-col gap-6">
        ...
        {/* KPIs row */}
        <div className="grid grid-cols-4 gap-3">
          <KpiCard label="Taux de confirmation" value={confRate + "%"} sub={`${confirmedOrders.length} / ${DEMO_ORDERS.length} commandes`} />
          <KpiCard label="Taux de livraison" value={deliveryRate + "%"} sub={`${deliveredOrders.length} livrées sur confirmées`} />
          <KpiCard label="Taux de rejet" value={DEMO_ORDERS.length > 0 ? ((rejectedOrders.length / DEMO_ORDERS.length) * 100).toFixed(1) + "%" : "—"} sub={`${rejectedOrders.length} rejetées`} />
          <KpiCard label="Sans réponse" value={DEMO_ORDERS.length > 0 ? ((noAnswerOrders.length / DEMO_ORDERS.length) * 100).toFixed(1) + "%" : "—"} sub={`${noAnswerOrders.length} sans réponse`} />
        </div>
        ...
        <div>
          <h2 className="text-xs font-semibold mb-3" style={{ color: "var(--muted-foreground)", letterSpacing: "0.06em", textTransform: "uppercase" }}>
            Produits les plus commandés (données démo)
          </h2>
          <div className="bg-card rounded" style={{ border: "1px solid var(--border)" }}>
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  {["Rang", "Produit", "Catégorie", "Commandes (démo)", "Stock"].map((h) => (
                  ...
                  <tr key={p.id} style={{ borderBottom: i < 4 ? "1px solid var(--border)" : undefined }}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <img src={p.images[0]} alt={p.name} className="w-8 h-8 rounded object-cover" style={{ border: "1px solid var(--border)" }} />
                        <span className="font-medium text-sm">{p.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs" style={{ color: "var(--muted-foreground)" }}>{p.category}</td>
                    <td className="px-4 py-3 font-mono text-sm">{Math.floor(Math.random() * 40) + 10}</td>
