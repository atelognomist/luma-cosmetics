...
import { ANALYTICS_DAILY, formatPrice, relativeTime, orderTotal } from "../lib/data";
...
export default function Dashboard() {
  ...
  const todayOrders = orders.filter((o) => new Date(o.createdAt) > new Date(Date.now() - 86400000));
  const pending = orders.filter((o) => o.status === "new" || o.status === "no_answer" || o.status === "calling");
  const confirmed = orders.filter((o) => ["confirmed", "preparing", "ready", "sent", "picked_up", "out_for_delivery", "delivered"].includes(o.status));
  const delivered = orders.filter((o) => o.status === "delivered");
  const revenue = delivered.reduce((s, o) => s + orderTotal(o as any), 0);
  const confRate = orders.length > 0 ? Math.round((confirmed.length / orders.length) * 100) : 0;
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-xs font-mono font-medium">{formatPrice(orderTotal(o as any))}</span>
                        </div>
                      </div>
